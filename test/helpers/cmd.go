...
// SendToLog writes the debug message for the running
// command, if the quietMode argument is true will print only the command and
// the exitcode.
func (res *CmdRes) SendToLog(logger *logrus.Entry, quietMode bool) {
	if quietMode {
		logformat := "cmd=%q exitCode=%d duration=%s\n"
		logger.Infof(logformat, logformat, res.cmd, res.GetExitCode(), res.duration)
		return
	}
