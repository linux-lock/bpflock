...
func (s *LocalExecutor) setBasePath() error {
	wd, err := os.Getwd()
	if err != nil {
		return err
	}
...
// ExecWithSudo returns the result of executing the provided cmd via
// sudo.
...
// Exec returns the results of executing the provided cmd.
...
// ExecContext returns the results of executing the provided cmd.
func (s *LocalExecutor) ExecContext(ctx context.Context, cmd string, options ...ExecOptions) *CmdRes {
	...
	res.SendToLog(s.logger, ops.SkipLog)
	return &res
}
