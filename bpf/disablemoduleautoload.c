...
#include <time.h>
#include <bpf/bpf.h>
#include "disablemoduleautoload.skel.h"
#include "trace_helpers.h"
#include "bpflock_utils.h"

static const char pin_path[] = "/sys/fs/bpf/bpflock/disable-module-autoload";

int main(int argc, char **argv)
{
	struct disablemoduleautoload_bpf *skel;
	...
	skel = disablemoduleautoload_bpf__open();
	if (!skel) {
		fprintf(stderr, "error: failed to open BPF skelect\n");
		return 1;
	}

	err = disablemoduleautoload_bpf__load(skel);
	if (err) {
	...
	printf("%s: success: module autoload disabled - pinned file '%s', remove to re-enable\n",
	       LOG_BPFLOCK, pin_path);

cleanup:
	bpf_link__destroy(link);
	//bpf_link__destroy(link);
	disablemoduleautoload_bpf__destroy(skel);
